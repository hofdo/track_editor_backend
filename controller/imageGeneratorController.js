/**
 * Track Generator Controller
 * Handles the requests for Track and Track Piece Generation
 */
...
//Load the config-methods from the track-piece-schema directory
 ...
 * @returns {Buffer} : Will be returned as a Buffer
 */
...
/**
 * This function handles the request and decides which track piece need to be generated
 * @param type: The type of the track piece
 * @param track_id: The track id the track piece should have
 * @param lanes: The amount of lanes the track piece should have
 * @param left: (Optional) This parameter only counts for the junction track piece and defines the amount of lanes that are on the left side of the track piece
 * @param right: (Optional) This parameter only counts for the junction track piece and defines the amount of lanes that are on the right side of the track piece
 */
...
/**
 * This function draws the track out of all the single track pieces
 * @param grid_items: A list of all the track pieces that are in the track
 * @param rows: How many rows the picture should have
 * @param cols: How many cols the picture should have
 *
 */
...
exports.drawImage = async function drawImage(grid_items, rows, cols) {
    ...
    for (let grid of grid_items) {
        if (grid.type === "junction"){
            grid_data = await mongoController.findTrack(grid.item.track_id, grid.type, grid.item.lanes, grid.item.left, grid.item.right)
        }
        else {
            grid_data = await mongoController.findTrack(grid.item.track_id, grid.type, grid.item.lanes)
        }
